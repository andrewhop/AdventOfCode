...
use aoc2024::day22::{
    day22_part1, day22_part1_parallel, day22_part1_simd, day22_part1_simdish, day22_part2,
    day22_part2_array_native_ring, day22_part2_arrayish, day22_part2_std, step, step_shift,
};
...
fn day22_bench(c: &mut Criterion) {
    ...
    // M1 time:   [1.0701 ms 1.0752 ms 1.0802 ms]
    // G4 time:
    c.bench_function("day22_part1_simple", |b| {
        b.iter(|| day22_part1(black_box(&input)))
    });

    // M1 time:   [2.0775 ms 2.0867 ms 2.0976 ms]
    // G4 time:
    c.bench_function("day22_part1_simdish", |b| {
        b.iter(|| day22_part1_simdish(black_box(&input)))
    });

    // M1 time: time:   [4.1629 ms 4.2006 ms 4.2478 ms]
    // G4 time:
    ...
    // M1 time: time:   [4.1629 ms 4.2006 ms 4.2478 ms]
    // G4 time:
    c.bench_function("day22_part1_parallel", |b| {
        b.iter(|| day22_part1_parallel(black_box(&input)))
    });
    ...
    // M1 time: time:   [13.136 ms 13.163 ms 13.192 ms]
    c.bench_function("day22_part2_array_native_ring", |b| {
        b.iter(|| day22_part2_array_native_ring(black_box(&input)))
    });

    // M1 time:   [13.222 ms 13.270 ms 13.323 ms]
    c.bench_function("day22_part2_array_native_nums", |b| {
        b.iter(|| day22_part2_array_native_ring(black_box(&input)))
    });

    // M1 time:   [471.21 ps 472.68 ps 474.49 ps]
