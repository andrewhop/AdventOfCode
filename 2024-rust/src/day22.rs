...
pub fn day22_part2_array(input: &[u8]) -> u16 {
    let ascii_str = std::str::from_utf8(input).expect("input was not UTF8 string");

    let mut overall_results: Vec<u16> = vec![0; MAX_VALUE];
    let mut overall_max: u16 = 0;
    for buyer_seed in ascii_str.lines() {
        let mut buyer_results: Vec<bool> = vec![false; MAX_VALUE];
        let mut change_seq: Vec<i8> = Vec::new();
        let mut next = buyer_seed.parse::<u64>().expect("invalid seed");
        for _ in 0..2_000 {
            let previous_price: i8 = (next % 10) as i8;
            next = step(next);
            ...
            change_seq.push(price_diff);
            if change_seq.len() == 4 {
                let sequence_key = seq_to_num(&change_seq);
                if !buyer_results[sequence_key] {
                    buyer_results[sequence_key] = true;
...
mod test {
    ...
    #[test]
    fn util_test() {
        assert_eq!(seq_to_num(&[-9, -9, -9, -9]), 0);
        assert_eq!(seq_to_num(&[0, 0, 0, 0]), 9999);
        assert_eq!(seq_to_num(&[9, 9, 9, 9]), 19998);
        assert_eq!(seq_to_num(&[9, 9, 9, 9]), MAX_VALUE);
    }
    ...
    fn part2_tests() {
        ...
        // assert_eq!(day22_part2_array(&sample), 23);
        ...
        assert_eq!(day22_part2_array(&input), 1568);
    }
}
